...
Page({
  ...
  // 跳转到对应报销
  toExpend: function (e) {
    let expend_name = e.currentTarget.dataset.index1;
    let bx_code = e.currentTarget.dataset.index2;
    wx.navigateTo({
      url: '../../../packageC/pages/expend-detail/expend-detail?name=' + expend_name+'&code='+ bx_code,
    })
  },
  ...
  onUnload: function () {
    ...
    if(pages.length >= 4){
      if (pages.length > 4 && pages[3].route == 'packageC/pages/expend-detail/expend-detail') {
        wx.navigateBack({
          delta:2,
        })     
     }else if(pages.length > 4 && pages[1].route == 'packageA/pages/bill-manage/bill-manage'){
       wx.navigateBack({
         delta:2,
       })
     }else if(pages.length > 4 && pages[1].route == 'packageB/pages/contract-manage/contract-manage'){
       
     }else if(pages.length = 4 && pages[3].route == 'packageA/pages/bill-relate-contract/bill-relate-contract'){
       wx.navigateBack({
