...
class department_list02 extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            // 控制对话框的显示.
    ...
    componentDidMount() {
        ...
        dispatch({
            ...
            payload: {
                breadcrumb: { 
                    name: '系统管理', href: '', 
                    name01: '组织部门管理', href01: '/fht/systemManagement/department/department_list.html', 
                    name02: location.query.dept_name, href02: '', 
                    name03: location.query.dept_name01, href03: '', 
                    detail: location.query.dept_name02 },
                menu: {
        ...
        dispatch({
            type: 'department_model/childDepartmentList',
            payload: { 
                p_dept_no: location.query.p_dept_no02, 
             },
        });
    }
    ...
    getAddData = value => {
        const { dispatch, location } = this.props;
        dispatch({
            type: 'department_model/childAddDepartment',
            payload: { 
                formdata: value, 
                p_dept_no: location.query.p_dept_no02,
                dept_name: location.query.dept_name02, 
            },
            callback: (code) => {
                if (code === 0) {
                    this.setState({
    ...
    getUpdateData = (value, dept_code) => {
        const { dispatch, location } = this.props;
        dispatch({
            type: 'department_model/childUpdate',
            payload: { formdata: value, dept_code, p_dept_no: location.query.p_dept_no02 },
            callback: (code) => {
                if (code === 0 || code === 2) {
                    dispatch({
                        type: 'department_model/updateState',
    ...
    deleteDept = (dept_code) => {
        const { dispatch, location } = this.props;
        dispatch({
            type: 'department_model/childDelete',
            payload: { dept_code, p_dept_no: location.query.p_dept_no02 },
        });
    }

    // 下级部门.
    toFourView = (code, name) => {
        const { location } = this.props;
        router.push({
            pathname: '/fht/systemManagement/department/department_list04',
            query: {
                p_dept_no03: code,
                dept_name03: name,
                p_dept_no02: location.query.p_dept_no02,
                dept_name02: location.query.dept_name02,
                p_dept_no01: location.query.p_dept_no01,
                dept_name01: location.query.dept_name01,
                p_dept_no: location.query.p_dept_no,
                dept_name: location.query.dept_name,
            },
        });
    }
    ...
    render() {
        ...
        const columns = [
            ...
            },
            {
                ...
                render: (text, record) => (
                    <span>
                        <Button size="small" type="link" onClick={this.toFourView.bind(this, record.dept_code, record.name)}>下级部门</Button>
...
export default department_list02;